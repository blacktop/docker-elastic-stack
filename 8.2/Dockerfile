...
RUN apk add --no-cache -t .build-deps wget ca-certificates \
	...
	&& echo "Download c..." \
	&& wget --progress=bar:force -O elasticsearch-$STACK.tar.gz https://artifacts.elastic.co/downloads/elasticsearch/elasticsearch-$STACK-linux-x86_64.tar.gz \
	...
	https://artifacts.elastic.co/downloads/logstash/logstash-$STACK-linux-x86_64.tar.gz \
