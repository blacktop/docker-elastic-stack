...
RUN apk add --no-cache openjdk17-jre tini su-exec

ENV STACK 8.2.3

RUN apk add --no-cache libzmq bash nodejs nginx apache2-utils openssl nss libc6-compat
RUN apk add --no-cache -t .build-deps wget ca-certificates \
	&& set -x \
	&& cd /tmp \
	&& echo "Download Elastic Stack ======================================================" \
	&& echo "Download c..." \
	&& wget --progress=bar:force -O elasticsearch-$STACK.tar.gz https://artifacts.elastic.co/downloads/elasticsearch/elasticsearch-$STACK-linux-aarch64.tar.gz \
	&& tar -xzf elasticsearch-$STACK.tar.gz \
	&& mv elasticsearch-$STACK /usr/share/elasticsearch \
	&& echo "Download Kibana..." \
	&& wget --progress=bar:force -O kibana-$STACK.tar.gz https://artifacts.elastic.co/downloads/kibana/kibana-$STACK-linux-aarch64.tar.gz \
	&& tar -xzf kibana-$STACK.tar.gz \
	&& mv kibana-$STACK /usr/share/kibana \
